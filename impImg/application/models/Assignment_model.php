		...
		public function add_or_edit()
		{
			...
			if(empty($_POST['ID']))
			{
				...
				foreach ($students as $key_st => $value_st) {
					$stu_asgn = $this->form_model->add(array("table"=>"SA","columns"=>array('student_ID'=>$value_st,'assignment_ID'=>$this->db_library->find_max_id('AS'),'subject_ID'=>$_POST['subject_ID'],'chapter'=>$_POST['chapter'],'topic'=>$_POST['topic'],'branch_ID'=>$_POST['branch_ID'],'asgn_status'=>'not_submitted')));
				}
			}
			else
			{
			...
			if($result === TRUE)
			{
				$res['id'] = $this->db_library->find_max_id('AS');
				$res['stat'] = TRUE;
