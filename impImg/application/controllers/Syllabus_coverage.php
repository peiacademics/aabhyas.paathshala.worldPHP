<?php
	...
	{
		$res = $this->syllabus_coverage_model->get_show_data(array('SYC'=>array('branch_ID'=>$branch_ID,'subject_ID'=>$subject_ID)),array('chapter','topic','date','>>Name:>fr>SB>name:ID=^subject_ID^','ID'));
		foreach ($res['data'] as $key => $value) {
			if (strpos($value[4], '/add') !== false) {
				$res['data'][$key][4] = str_replace('/add', '/add/'.$branch_ID, $value[4]);
			}
		}
