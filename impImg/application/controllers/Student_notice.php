<?php
	...
	public function get_show_data($branch_ID = NULL)
	{
		...
		{
			$res = $this->student_notice_model->get_show_data(array('SN'=>array('branch_ID'=>$branch_ID,'student_ID LIKE'=>'%'.$this->data['Login']['ID'].'%')),array('title','date','description','ID'));
		}
		else
		{
			$res = $this->student_notice_model->get_show_data(array('SN'=>array('branch_ID'=>$branch_ID)),array('title','date','description','ID'));
