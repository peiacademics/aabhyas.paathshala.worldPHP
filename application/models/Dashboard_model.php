<?php
	...
	public function get_topic_details()
	{
		...
		$this->load->library('encrypt');
		$my_config = $this->config->item('skyq');
		$decrypt = $this->encrypt->decrypt_string(base64_decode(urldecode($_POST['url_data'])),$my_config['app_ie']);
		$exp = explode('^',$decrypt);
		$package_detail = $this->fetch_model->show(array('APGD'=>array('subject_ID'=>$exp[0],'lesson_ID'=>$exp[1],'topic_ID'=>$exp[2],'package_ID'=>$exp[3])));
		if(!empty($package_detail))
		{
			foreach ($package_detail as $key => $value) {
				$explode = explode('SK',$value['reference_ID']);
				$value['title'] = $this->str_function_library->call('fr>'.$explode[0].'>name:ID=`'.$value['reference_ID'].'`');
				$encrypt = $decrypt.'^'.$value['reference_ID'];
				$value['href'] = urlencode(base64_encode($this->encrypt->encrypt_string($encrypt,$my_config['app_ie'])));
				$data[$value['type']][] = $value;
			}
			if(array_key_exists('test',$data))
			{
				$tests = implode('||',array_column($data['test'], 'reference_ID'));
				$data['test_history'] = $this->fetch_model->show(array('ATS'=>array('student_ID'=>$this->session_library->get_session_data('ID'),'test_ID'=>$tests)),array('datetime','ID','total_marks','marks_obtained','test_ID'));
				if(!empty($data['test_history']))
				{
					foreach ($data['test_history'] as $thk => $thv) {
						$explod = explode('SK',$thv['test_ID']);
						$data['test_history'][$thk]['name'] = $this->str_function_library->call('fr>'.$explod[0].'>name:ID=`'.$thv['test_ID'].'`');
					}
				}
			}
			if(array_key_exists('assignment', $data))
			{
				$assignments = implode('||',array_column($data['assignment'], 'reference_ID'));
				$data['assignment_history'] = $this->fetch_model->show(array('AAS'=>array('student_ID'=>$this->session_library->get_session_data('ID'),'assignment_ID'=>$assignments)),array('datetime','ID','total_marks','marks_obtained','>>name:>fr>AQS>name:ID=^assignment_ID^'));
			}
		}
		$tests1 = $this->fetch_model->show(array('AM'=>array('subject_ID'=>$exp[0],'lesson_ID'=>$exp[1],'topic_ID'=>$exp[2])));
		$tests2 = $this->fetch_model->show(array('AQT'=>array('subject_ID'=>$exp[0],'lesson_ID'=>$exp[1],'topic_ID'=>$exp[2])));
		// $tests = array_merge($tests1,$tests2);
		// print_r($tests);
		// $assignments = $this->fetch_model->show(array('AQS'=>array('subject_ID'=>$exp[0],'lesson_ID'=>$exp[1],'topic_ID'=>$exp[2],'package_ID'=>$exp[3])));

		// if(!empty($test_detail))
		// {
		// 	foreach ($test_detail as $key => $value) {
		// 		// $explode = explode('SK',$value['reference_ID']);
		// 		$value['title'] = $this->str_function_library->call('fr>'.$explode[0].'>name:ID=`'.$value['reference_ID'].'`');
		// 		$encrypt = $decrypt.'^'.$value['reference_ID'];
		// 		$value['href'] = urlencode(base64_encode($this->encrypt->encrypt_string($encrypt,$my_config['app_ie'])));
	...
	public function get_list()
	{
		...
		foreach ($packages_ID as $pkg_ID) {
			...
			$all_data[$this->str_function_library->call('fr>APG>name:ID=`'.$pkg_ID.'`')] = $new_data;
		}
